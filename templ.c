...
WARN_UNUSED static bool loop_stack_init(LoopStack* stack, size_t capacity);
WARN_UNUSED static bool loop_stack_push(LoopStack* stack, const TemplateValue* array, const char* item_name,
                                        const char* loop_start);
...
WARN_UNUSED static bool conditional_stack_push(ConditionalStack* stack, bool condition, const char* if_start);
            ...
            if (!should_skip) {
                ...
                *(end + 1) = '\0';
                ...
                if (current_loop && strcmp(name, current_loop->item_name) == 0) {
                    ...
                    // (Code to extract item from array remains the same)
                    // ...
                    switch (item.type) {
                        case TMPL_STRING:
                            item.value.str =
...
bool loop_stack_init(LoopStack* stack, size_t capacity) {
...
bool loop_stack_push(LoopStack* stack, const TemplateValue* array, const char* item_name,
                     const char* loop_start) {
...
bool conditional_stack_init(ConditionalStack* stack, size_t capacity) {
...
bool conditional_stack_push(ConditionalStack* stack, bool condition, const char* if_start) {
    if (stack->depth >= stack->capacity) {
        stack->capacity *= 2;
